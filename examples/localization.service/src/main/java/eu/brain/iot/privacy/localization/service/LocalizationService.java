 ...
 ******************************************************************************/
package eu.brain.iot.privacy.localization.service;
...
@Component(immediate=true, service= {SmartBehaviour.class,LocalizationService.class})
@SmartBehaviourDefinition(consumed= {LocalizationEventDTO.class},filter="", 
                         author="LINKS", name ="Localization Service Behavior", 
                         description="Edge node for the service that locate the users.")
public class LocalizationService implements SmartBehaviour<LocalizationEventDTO>{
	...
	public void start(BundleContext context){
		System.out.println("Service: I am the Localization Service");
	...
	public void notify(LocalizationEventDTO event) {
		System.out.println("Service: event received with token: "+event.token);
		...
		try {
			clientMQTT.publish("/brainiot/"+event.deviceId+"/localization",	eventToSend.getBytes(), 0, false);
