 ...
 ******************************************************************************/
package eu.brain.iot.privacy.autopilot.service;
...
@Component(immediate=true, service= {SmartBehaviour.class,AutopilotService.class})
@SmartBehaviourDefinition(consumed= {LocalizationEventDTO.class},filter="", 
                         author="LINKS", name ="Autopilot Service Behavior", 
                         description="Service of the AUTOPILOT projects.")
public class AutopilotService implements SmartBehaviour<LocalizationEventDTO>{
	...
	public void start(BundleContext context){
		System.out.println("AUTOPILOT Service: I am the AUTOPILOT Service");		
	...
	public void notify(LocalizationEventDTO event) {
		System.out.println("AUTOPILOT Service: event received with token: "+event.token);
		...
		try {
			clientMQTT.publish("/brainiot/AUTOPILOT/"+event.deviceId+"/localization",	eventToSend.getBytes(), 0, false);
