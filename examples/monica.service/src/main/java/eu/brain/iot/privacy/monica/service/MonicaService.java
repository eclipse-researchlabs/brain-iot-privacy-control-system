package eu.brain.iot.privacy.monica.service;
...
@Component(immediate=true, service= {SmartBehaviour.class, MonicaService.class})
@SmartBehaviourDefinition(consumed= {LocalizationEventDTO.class},filter="", 
                         author="LINKS", name ="Monica Service Behavior", 
                         description="Service for the MONICA project")
public class MonicaService implements SmartBehaviour<LocalizationEventDTO>{
	...
	public void start(BundleContext context){
		System.out.println("Service: I am the MONICA Service");		
	...
	public void notify(LocalizationEventDTO event) {
		System.out.println("MONICA Service: event received with token: "+event.token);
		...
		try {
			clientMQTT.publish("/brainiot/MONICA/"+event.deviceId+"/localization",	eventToSend.getBytes(), 0, false);
